...
import { ConfigState, MODELS } from "@/components/configurator/Model";
...
export default function Home() {
  ...
  const [config, setConfig] = useState<ConfigState>(() => {
    ...
    // Pre-fill default colors for the initial model
    const initialModel =
      MODELS.find((m) => m.id === baseConfig.selectedModel) || MODELS[0];
    initialModel.parts.forEach((part) => {
      baseConfig.partColors[part.id] = baseConfig.mainColor;
    });
  ...
  // Helper to change models and ensure part colors are initialized correctly
  const handleModelChange = (modelId: string) => {
    const model = MODELS.find((m) => m.id === modelId) || MODELS[0];
    setConfig((prev) => {
      const nextColors = { ...prev.partColors };
      model.parts.forEach((part) => {
        if (!nextColors[part.id]) {
          nextColors[part.id] = prev.mainColor;
        }
      });
      return {
        ...prev,
        selectedModel: modelId,
        partColors: nextColors,
      };
    });
  };
  ...
  const handleShare = async () => {
    const configString = btoa(
      JSON.stringify({
        ...
        selectedTexture: config.selectedTexture,
